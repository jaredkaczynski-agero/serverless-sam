...
class FunctionConverter {
  ...
  serverlessFunctionToSam(resourceName, serverlessFunction) {
    ...
    var codeUri = this.getCodeUri(serverlessFunction);
    codeUri = codeUri.split('.serverless')[0];
    if (codeUri.startsWith('/mnt/')){
      codeUri = codeUri.substring(5)
      codeUri = codeUri.substring(0,1) + ':' + codeUri.substring(1)
    }
    codeUri = codeUri.replace(new RegExp('/'.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'g'), '\\');    
    ...
    let layers = Object.assign([], serverlessFunction.layers);
    for(var i = 0; i< layers.length; i++){
      samFunctionBuilder = samFunctionBuilder.addLambdaLayer(layers[i]);
    }
